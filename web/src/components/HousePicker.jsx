...
export default function HousePicker({ userRow, onJoined, onBack }) {
  ...
  const filtered = publicHouses.filter((h) => h.name.toLowerCase().includes(search.toLowerCase()));
  ...
  async function createHouse(e) {
    ...
    const { data: created, error } = await supabase
      ...
      .insert({
        ...
        join_password: !newIsPublic ? newPassword : null,
      })
      .select()
      .single();

    if (error || !created) {
      setCreateError(error?.message || 'Failed to create house.');
      setCreating(false);
      return;
    }
    ...
    await supabase
      .from('house_users')
      .insert({ house_id: created.id, user_id: userRow.id, role: 'owner' });
    onJoined(created);
  }
        ...
        <div className="house-search-row">
          <input
            ...
            onChange={(e) => {
              ...
              setPrivateResult(null);
              setPrivateSearched(false);
            }}
            placeholder="Search houses…"
          />
          <button className="btn" type="button" onClick={findPrivateHouse}>
            Find private house
          </button>
        </div>
